...
class UCCASS_EditSurvey extends UCCASS_Main
{
    ...
    function _processProperties($sid)
    {
        ...
        if(empty($pr['error']))
        {
        	//by Yan. add read_password, mail_subject, mail_receiver, default_referrer,
        	//by Yan.     display_state
            $query = "UPDATE {$this->CONF['db_tbl_prefix']}surveys SET name={$pr['input']['name']}, start_date={$pr['input']['start']},
                      ...
                      read_password = {$pr['input']['read_password']},
                      mail_subject = {$pr['input']['mail_subject']},
                      mail_receiver = {$pr['input']['mail_receiver']},
                      default_referrer = {$pr['input']['default_referrer']},
                      display_state = {$pr['input']['display_state']},
                      on_top = {$pr['input']['on_top']},
                      region = {$pr['input']['region']},
                      key_desc = {$pr['input']['key_desc']},
...
//exit;		
    ...
    function _validateProperties($sid)
    {
        ...
        //by Yan. Get read_password from submission
        $input['read_password'] = $this->SfStr->getSafeString($_REQUEST['read_password'],SAFE_STRING_DB);
        //by Yan. Get subject and receivers for sending email.
        $input['mail_subject'] = $this->SfStr->getSafeString($_REQUEST['mail_subject'],SAFE_STRING_DB);
        $input['mail_receiver'] = $this->SfStr->getSafeString($_REQUEST['mail_receiver'],SAFE_STRING_DB);
        
        //by Yan. On top to make a survey be on top of the list.
        $input['on_top'] = $this->SfStr->getSafeString($_REQUEST['on_top'],SAFE_STRING_DB);
        //by Yan. add region to make a survey to be grouped by region (1:all, 2:taipei, 3:taichung, 4: kaohsuang)
        $input['region'] = $this->SfStr->getSafeString($_REQUEST['region'],SAFE_STRING_DB);
				//by Yan. On top to make a survey be on top of the list.
        $input['key_desc'] = $this->SfStr->getSafeString($_REQUEST['key_desc'],SAFE_STRING_DB);

        //by Yan. Get default_referrer to fill in result list.
        $input['default_referrer'] = $this->SfStr->getSafeString($_REQUEST['default_referrer'],SAFE_STRING_DB);

        //by Yan. Get display_state to fill in result list.
        $input['display_state'] = $this->SfStr->getSafeString($_REQUEST['display_state'],SAFE_STRING_DB);
