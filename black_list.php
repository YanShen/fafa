...
class BlackList extends UCCASS_Main {
	
	Function initial() {
        $data = array();
        $template = 'admin.tpl';

        $admin_priv = $this->_CheckLogin(0,ADMIN_PRIV,'black_list.php');
	...
	Function listNames() {
		$rs = $this->Query("SELECT * from {$this->CONF['db_tbl_prefix']}black_list");
		if($rs === FALSE)
		{ $this->error("Error loading Black_List " . $this->db->ErrorMsg()); return; }
	...
	Function insertName($names) {
		...
		foreach($nameArray as $name) {
			...
			$rs = $this->Query("insert into {$this->CONF['db_tbl_prefix']}black_list (name) values ('$name')");
			if($rs === FALSE)
			{ $this->error("Error inserting Black_List " . $this->db->ErrorMsg()); $result = false; }
			else {
				echo $name." 成功<br>\n";
			}
		} 
	...
	Function delete($ids) {
		foreach($ids as $id) {
			$rs = $this->Query("delete from {$this->CONF['db_tbl_prefix']}black_list where id=$id");
			if($rs === FALSE)
			{ $this->error("Error delete Black_List " . $this->db->ErrorMsg()); return false; }
		} 
		return true;
	}
	
	Function updateName($origId, $newName) {
		$rs = $this->Query("update {$this->CONF['db_tbl_prefix']}black_list set name='$newName' where id=$origId");
		if($rs === FALSE)
		{ $this->error("Error update Black_List " . $this->db->ErrorMsg()); return false; }
		else {
			return true;
		}
	}
...
<table align="center" cellpadding="0" cellspacing="0">
  <tr class="grayboxheader">
    ...
    <td background="<?=$bl->CONF["images_html"]?>/box_bg.gif">黑名單</td>
...
<table align="center" class="bordered_table">
  ...
  <tr> 
   	<td>
		<form name="form1" action="black_list.php" method="POST">
